...
void ROS2Visualizer::setup_subscribers(std::shared_ptr<ov_core::YamlParser> parser) {
  ...
  _node->declare_parameter<std::string>("topic_imu", "/leap/ins570_imu");
  ...
  if (_app->get_params().state_options.num_cameras == 2) {
    ...
    _node->declare_parameter<std::string>("topic_camera" + std::to_string(0), "/leap/sensor/multicamera/image");
    _node->get_parameter("topic_camera" + std::to_string(0), cam_topic0);
    _node->declare_parameter<std::string>("topic_camera" + std::to_string(1), "/leap/sensor/multicamera/image");
    ...
    for (int i = 0; i < _app->get_params().state_options.num_cameras; i++) {
      ...
      _node->declare_parameter<std::string>("topic_camera" + std::to_string(i), "/leap/sensor/multicamera/image");
